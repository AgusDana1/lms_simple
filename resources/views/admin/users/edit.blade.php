<x-app-layout pageHeader="Edit Pengguna">
        ...
        <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-8">
            <div class="mb-6 pb-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Perbarui data profil, peran sistem, atau status keaktifan akun</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="identifier_number" 
                            value="{{ old('identifier_number', $user->studentProfile?->student_number ?? $user->lecturerProfile?->lecturer_number) }}"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="study_program" 
                            value="{{ old('study_program', $user->studentProfile?->study_program ?? $user->lecturerProfile?->study_program) }}"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Kata Sandi Baru (Kosongkan jika tidak diubah)</label>
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600">
                        <span class="text-xs font-bold text-slate-700">Status Akun Aktif</span>
                    </label>
                </div>
